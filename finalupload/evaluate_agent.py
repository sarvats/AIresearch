...
def evaluate_agent(model_path, ticker='AAPL', start_date='2023-01-01', end_date='2023-12-31'):
    ...
    while not done:
        ...
        total_reward += reward
        if step_count % 20 == 0:
            print(f"[Step {step_count:>4}] | Action: {action[0]:>6.3f} | Reward: {reward:>8.5f} | Total: {total_reward:>9.5f}")
    ...
    print(f"Agent ROI                : {roi_agent:6.2f}%")
    print(f"Buy & Hold Final Value   : ${buy_hold_value:,.2f}")
    print(f"Buy & Hold ROI           : {roi_buy_hold:6.2f}%")
    print(f"Outperformance           : {roi_agent - roi_buy_hold:6.2f}%")

    # Save evaluation history
    pd.DataFrame(env.history).to_csv('evaluation_results.csv', index=False)
